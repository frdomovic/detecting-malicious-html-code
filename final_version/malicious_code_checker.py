...
for url in url_collection.find({}).limit(20000).skip(20000):
        #
        #
                ...
                if(firsthash != secondhash):
                    ...
                    if(first_parsed_page != second_parsed_page):
                        ...
                        if(continue_search):
                            ...
                            # 1. ako je u listi novo dodanih elemenata link/url (+2)
                            # 2. ako je u bilo kojem tagu atribut "hidden" ili više njih (+1)
                            ...
                            for i in soupEnd.findAll({}):
                                if(len(i.attrs) > 0 ):
                                    for key in i.attrs:
                                        lista = ["www","www.",".com",".org","https://","http://","ftp://","gopher://","file://"]
                            ...
                            for key in css_vals_cist:
                                    if key.strip() in bad_css:
                                        if(key.strip() == "z-index"):
                                            ...
                                            if( val < -500 or val > 2500 or val == 0):
                                                security_index +=1
                                            ...
                                            if(float(css_vals_cist[key]) < 1):
                                                security_index +=1
                            #
                            #za dodavanje bilo kakvog linka dodajemo +2 zbog toga što
                            ...
                            if(security_index >= 3):
                                ...
                                with open(name,"w") as f:
                                    f.write(difference)
                                attr_difference3 = "\n".join(difflib.Differ().compare(parsed_page_attrs_old.split("\n"),parsed_page_attrs_new.split("\n")))
                                diffCharArray3 = attr_difference3.split("\n")
                                new_added_attrs3 = []
                                #
                                #
                                #10. dodavanje samo linija koje su dodane u novoj verziji tj. koje imaju + na početku
                                ...
                                #
                                #
                                for line in diffCharArray3:    
                                    if(len(line) > 1):
                                        if "+"  == line[0]:
                                            if "<" in line or ">" in line:
                                                if "<" not in line:
                                                    temp = line.replace("+","")
                                                    new_added_attrs3.append("<"+temp)
                                                elif ">" not in line:
                                                    temp = line.replace("+","")
                                                    new_added_attrs3.append(">"+temp)
                                                else:
                                                    new_added_attrs3.append(line.replace("+",""))
                                end2 = ("".join(new_added_attrs3))
                                soupEnd2 = bs(end2, 'lxml')
                                
                                with open("./DIFFERENCES_ONLY/"+str(index)+".html","w") as x:
                                    x.write(soupEnd2.prettify())
